...
class StellarBody:
    ...
    def __init__(self, name, mass: Q_, radius: Q_ = np.nan * ureg.R_s, luminosity: Q_ = np.nan * ureg.L_s,
        ...
        self.parent: Union[Star, Planet, StellarBinary] = parent
    ...
    def __repr__(self, precision=4) -> str:
        ...
        keys.sort()
        for key in keys:
            if key not in ['image_array', 'parent', 'farthest_parent', 'insolation_model']:
...
class Planet(StellarBody):
    ...
    def calculate_suggested_spin_period(self) -> Q_:
        """
        From our own solar system averages. Does not account for atmospheric tides (like Venus) or tidal locking
